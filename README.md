...
Spain is the first area built by the agent team from those documents: the [Spain world](docs/spain-world.md) records the baseline, the blueprint, the picture acceptance and the checks; the [Spain research](docs/spain-research.md) holds the sourced facts; the [Spain image brief](docs/spain-image-brief.md) is the final brief the pictures were generated from.
...
The [quality baseline](docs/quality-baseline.md) records what those harnesses enforce and the numbers a session must hold, and the
[Spain retrospective](docs/spain-world.md#retrospective-2026-09-16) lists the twelve things ten minutes of the owner's looking found after the area had been called done.
...
dastikhan…) that serve as card badges. The China table is 112 × 84 with its centre at x −6 (`cx` in the world spec). Since 2026-09-11 the map is four
tight clusters with open ground between them: Sichuan's street and market ring in the centre-west (the hotpot house
behind it), the northern town in the north-east packed around the palace on its own street (three rows: workshops
by the peaks, the wheat and grills on the street, dumplings and the courtyard compound below the gate), the Jiangnan
water town on both banks of the stone bridge with its countryside east and south, and the oasis strip on the west.
Roads connect them: the gate road climbs from the Sichuan street to the palace gate, a lane drops to the bridge, and
the road west runs along the river through the mountain gap to the oasis road. The river runs flush to both edges. Northern China's twelve rooms arrived the same day (`~/Downloads/additional game asset/northern`, cut with the
