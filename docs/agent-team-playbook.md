...
- **Every cluster is compact.** Each area doc states every cluster's centre and radius (about 9, the China and Spain size), and every object of the cluster stands within that radius of its centre; every object belongs to exactly one cluster, and each cluster stands on a ground tint of its own. `<id>-world.mjs` checks it. Lead ruling, 2026-09-23: the shared-ground pass spread clusters and the owner could not see them ("In China, regions are nicely clustered and separated; in the UK it can be messy"). A layout pass may move stands to clear rays, water or footprints, but it keeps each cluster together to do it
- **The clusters are separated.** No object of one cluster stands inside the convex hull of another cluster's objects, and the hulls of any two clusters are at least 8 apart, with open country, a road or water between them and no stand. `<id>-world.mjs` checks both and prints the ground left between the clusters' footprints. Same ruling and reason, 2026-09-23: the shared-ground pass spread clusters and the owner could not see them
- **No house and no stand building in or over any water.** Every decorative house and every stand's own building (its shop, oast, shed or engine house) stands wholly on dry ground, its footprint at least 1.0 from every water edge — sea, strait, inlet, river, pool and burn — and `<id>-world.mjs` tests the footprints of the houses and the buildings against every water body, not only the stands' vertices. Lead ruling, 2026-09-23, from the owner's note on the live Britain: "please make sure houses are not standing in the river"
...
- Decorative houses are capped at **five per area id**, and each one stands where it hides no stand: first on none of any clickable's ten arrival rays (the visibility line below) and never on the arrival camera's line to a stand. Nothing solid stands on a road centreline. Every stand keeps a clear corridor from the nearest road with 2.5 units clear in front of it (`<id>-world.mjs`). Lead ruling, 2026-09-23: this line used to ask for 12 to 16 houses, 1 to 3 per style. That number was retired by Spain's third pass on 2026-09-17, when the owner found stands hidden behind houses and Spain went from fourteen houses to five because almost no ground near a cluster lay outside every stand's approach; Thailand, Vietnam, Italy (4, 4 and 5 across its three area ids) and Britain have all shipped at five or fewer since. Fewer than five is a pass; a house that hides a stand is a fail whatever the count
...
- Every object has a tagline and a blurb with dated eras and local-script names. The blurb proper is three to five paragraphs; a room object's story depth is appended below it on the card and is not counted in those five. Lead ruling, 2026-09-23: Spain, Thailand, Vietnam and Britain ship four paragraphs of blurb and three of story depth, seven on the card, and Italy three and three, six; two Stage E reviews in a row had to ask whether a seven-paragraph card broke a five-paragraph line. The line now counts the blurb, which is what the band was always about
