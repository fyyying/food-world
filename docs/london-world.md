...
## Owner ruling, 2026-09-24: Britain becomes its own world, the UK

After the re-cluster pass went live the owner looked again on her phone: "still not clustered properly, it feels too crowded and no space between places, can't tell where one area belongs vs another. China has a clear area for Sichuan, separated by mountains from Xinjiang." Her decisions:

1. **Britain leaves Central Europe and becomes its own world, the UK.** A new world id (`uk`), its own plate on the landing page, `PUBLISHED_WORLDS` gains `uk` and drops `central-europe`. Budapest, the Alps and Georgia stay card-only in an unpublished `central-europe` world until they are built. Old `central-europe` links open the UK.
2. **The world can be bigger.** The whole table is the island: the continental strip east of x -31.4 goes, and the island grows to use the space (about twice its current 44 by 54 area, more if the separation needs it). The camera limits and fog follow the new size.
3. **Regions separated the China way.** The six clusters stay dense (the radius rule stands), but they are now set apart by landscape the visitor can read at a glance, not by 2 to 5 units of lawn: the Highlands and the Southern Uplands between the Firths and England, the Pennines between the Dales and the south, the North Downs and the Thames between the Weald and London, the moors and the Bristol Channel around the West Country, the river and its estuary between Westminster and the Docks. Target: the open ground between two clusters' footprint hulls at least as wide as China's narrowest gap between two areas (to be measured from the China world and written here), and never under 10.
