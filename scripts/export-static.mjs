...
const isChina = (r) => r.cuisine === "Chinese" || /mapo|sichuan|szechuan|hong shao/i.test(r.title) || r.cuisine === "Italian" || /italian|caprese|bolognese|lasagn|pizza|pesto/i.test(r.title) || r.cuisine === "Korean" || /korean|bulgogi|bibimbap|kimchi/i.test(r.title) || r.cuisine === "Mexican" || /mexican|taco|salsa|guacamole|carnitas|chili con carne/i.test(r.title) || ["Middle Eastern", "Lebanese", "Turkish"].includes(r.cuisine) || /shawarma|falafel|hummus|kebab|tabouli|tabbouleh|shakshuka/i.test(r.title) || ["Mediterranean", "Greek", "Spanish", "North African"].includes(r.cuisine) || /greek|tzatziki|halloumi|tagine|kupus/i.test(r.title) || r.cuisine === "Indian" || /tandoori|tikka|masala|dal\b|paneer|biryani|korma|naan/i.test(r.title) || r.cuisine === "Thai" || r.cuisine === "Vietnamese" || /banh mi|goi ga|pad thai/i.test(r.title) || r.cuisine === "American" || /pancake|waffle|burger|ribs|texas|smoked|chicken wings|slaw|banana nut|instant pot/i.test(r.title);
