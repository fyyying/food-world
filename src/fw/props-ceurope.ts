...
export function towerBridge(len = 8): P {
  ...
  for (const sd of [-1, 1]) { const x = sd * len * 0.28; for (const sz of [-1, 1]) { add(g, box(1.6, 6.5, 1.0, c), x, 3.25, sz * 1.45); for (let k = 0; k < 4; k++) add(g, box(0.3, 0.6, 0.04, "#5a6a7a"), x, 1.5 + k * 1.3, sz * 1.45 + 0.52); } add(g, box(1.6, 1.4, 3.9, c), x, 5.8, 0); add(g, box(1.8, 0.5, 4.1, c), x, 6.7, 0); add(g, cone(1.0, 1.6, CE.slate, 4), x, 7.7, 0).rotation.y = Math.PI / 4; for (const sx of [-1, 1]) for (const sz of [-1, 1]) add(g, cone(0.18, 1.0, c, 4), x + sx * 0.7, 7.3, sz * 1.6); add(g, box(2.0, 0.6, 4.2, "#8f857a"), x, 0.3, 0); }
...
export function chainBridge(len = 8): P {
  ...
  for (const sd of [-1, 1]) { const x = sd * len * 0.28; for (const sz of [-1, 1]) add(g, box(1.4, 4.5, 0.7, CE.stone), x, 2.25, sz * 1.05); add(g, box(1.4, 1.0, 2.8, CE.stone), x, 4.0, 0); add(g, box(1.6, 0.3, 3.0, CE.stone), x, 4.6, 0); add(g, box(1.6, 0.6, 3.0, "#8f857a"), x, 0.3, 0); }
...
export function alpineCows(): P {
  const g = group();
  const cows: P[] = [];
  for (let i = 0; i < 3; i++) { const c = cow(false, i === 0, "Muh! Moo!"); c.traverse((o) => { if (o instanceof THREE.Mesh) { const col = (o.material as THREE.MeshStandardMaterial).color.getHexString(); if (col !== "e9b8a5" && col !== "e8dcc2") (o.material as THREE.MeshStandardMaterial) = mat(col === C.cowBrown.replace("#", "") ? "#7a4a2a" : "#a8703a") as THREE.MeshStandardMaterial; } }); add(c.children[0], cyl(0.09, 0.1, 0.14, CE.gold, 8), 0.95, 0.55, 0); add(c.children[0], box(0.34, 0.08, 0.06, "#8a2a2a"), 0.95, 0.64, 0); c.position.set([-2.6, 0.9, 1.4][i], 0, [-0.4, 1.0, -1.6][i]); c.rotation.y = i * 1.3; g.add(c); cows.push(c); }
