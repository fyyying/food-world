...
type Room = { title: string; zh: string; caption: string; touches: Touch[]; steam?: [Point, Point]; fire?: [Point, Point]; night?: boolean };
const rooms: Record<string, Room> = {
  ...
  tr_kebab: {
    ...
    steam: [[.40,.66],[.47,.55]], fire: [[.30,.78],[.35,.63]],
    touches: [
...
function makeRoom(id: string, room: Room): SceneDef {
  ...
  const cfg: PaintedCfg = { id, folder: id, title: room.title, zh: room.zh, caption: room.caption,
    painting: true, night: room.night, hotspots, motes: 18, light: { x: 750, y: 280, color: 'rgba(255,210,150,0.16)' }, portrait: {} };
  if (room.steam) {
    const [wide, phone] = room.steam;
    cfg.steam = [{ x: wide[0] * 1600, y: wide[1] * 900, w: 42, rate: 4, a: .18 }];
    cfg.portrait!.steam = [{ ...pAt(id, ...phone), w: 22, rate: 3, a: .16 }];
  }
  if (room.fire) {
    const [wide, phone] = room.fire;
    cfg.fire = [{ x: wide[0] * 1600, y: wide[1] * 900, rx: 55, ry: 30 }];
    cfg.portrait!.fire = [{ ...pAt(id, ...phone), rx: 27, ry: 18 }];
  }
