...
export function snapshot(key: string, size = 192): string | null {
  ...
  if (hit) return hit;
  const build = ICONS[key] ?? ITALY_ICONS[key] ?? KOREA_ICONS[key] ?? MEXICO_ICONS[key] ?? MIDEAST_ICONS[key] ?? MED_ICONS[key] ?? INDIA_ICONS[key] ?? PROPS[key] ?? ITALY_PROPS[key] ?? KOREA_PROPS[key] ?? MEXICO_PROPS[key] ?? MIDEAST_PROPS[key] ?? MED_PROPS[key] ?? INDIA_PROPS[key];
