...
export function mountUi(h: UiHandlers) {
  ...
  function showObject(o: WorldObject, recipes: EnrichedRecipe[], allObjects: WorldObject[]) {
    ...
    const nextPlaces = (o.world === 'italy' ? ITALY_NEXT[o.id] ?? [] : o.world === 'middle-east' ? TURKEY_NEXT[o.id] ?? [] : o.world === 'mediterranean' ? SPAIN_NEXT[o.id] ?? [] : o.world === 'southeast-asia' ? (o.area === 'bangkok' ? THAILAND_NEXT[o.id] : VIETNAM_NEXT[o.id]) ?? [] : []).flatMap(id => allObjects.filter(p => p.id === id));
    el.className = "";
    el.innerHTML = `
