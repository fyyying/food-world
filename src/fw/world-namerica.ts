...
function layoutNamerica({ group, tickers, place, tint, TOP }: LayoutCtx) {
  ...
  const coast = (pts: [number, number][], cx: number, d: number) => pts.map(([x, z]) => { const edge = Math.abs(x) >= 38 || Math.abs(z) >= 28; if (edge) return [x, z] as [number, number]; return [x + Math.sign(cx) * -d, z] as [number, number]; });
  ...
  const pacific: [number, number][] = [[-38, -28], [-32, -28], [-32.5, -22], [-31.5, -14], [-32, -11], [-27.5, -10], [-26.5, -7], [-27.5, -4], [-32, -3], [-31.5, 4], [-32.5, 12], [-31.5, 20], [-32.5, 28], [-38, 28]];
  ...
  add(group, new THREE.Mesh(riverGeometry(miss, 6.2), mat("#eadfbd")), 0, 0.03, 0);
  ...
  tickers.push((t) => { const raw = (t * 0.012) % 2; const u = raw < 1 ? raw : 2 - raw; const uu = 0.05 + u * 0.8; const p = miss.getPointAt(uu), n = miss.getPointAt(uu + (raw < 1 ? 0.01 : -0.01)); steamer.position.set(p.x, TOP + 0.05, p.z); steamer.rotation.y = Math.atan2(n.x - p.x, n.z - p.z) - Math.PI / 2; wheel.rotation.x = t * 3; });
  ...
  for (const [x, z] of [[-13, -25], [-2, -26], [2, -8], [-1, -1], [14, -8], [10, -1]] as [number, number][]) place(tree("round", 1.0), x, z, x);
  ...
  const trail = new THREE.CatmullRomCurve3([[-4, 17.5], [-4, 24], [-9, 26.5], [-16, 22.5], [-20, 17], [-18, 13.5], [-14.5, 12.5], [-14.5, 17], [-10, 17]].map(([x, z]) => new THREE.Vector3(x, 0, z)), true);
  group.add(path([[-4, 17.5], [-4, 24], [-9, 26.5], [-16, 22.5], [-20, 17], [-18, 13.5], [-14.5, 12.5], [-14.5, 17], [-10, 17], [-4, 17.5]], 1.4, "#d9b98a"));
  ...
  for (const [x, z, rot, shirt] of [[-12, 0.5, 0.4, "#2a2a2e"], [13, 19.5, -0.6, NA.white]] as [number, number, number, string][]) { const h = horse(x < 0 ? "#c9b48c" : "#6b4a2c", shirt); place(h, x, z, rot); tickers.push((t) => h.userData.gait?.(t, 0.15)); }
  ...
  place(pumpjack(), 13, 22, 0.4); place(pumpjack(), 11, 26, -0.3);
  for (const [x, z, s] of [[-12, 23, 1.0], [-4, 25, 0.9], [2, 21, 1.1], [14, 18, 0.8]] as [number, number, number][]) place(saguaro(s), x, z, x);
  for (const [x, z] of [[-14, 7], [3, 7], [14, 15], [-4, 17], [2, 12]] as [number, number][]) place(tree("round", 0.9), x, z, x);
  ...
  group.add(path([[-26, -12], [-21, -15], [-15, -11], [-14, 2], [-16, 6], [-27, 6], [-24.5, -6], [-26, -12]], 1.8, "#c9c0a8"));
  ...
  const loops: [THREE.CatmullRomCurve3, [string, string][], number][] = [
    ...
    [new THREE.CatmullRomCurve3([[-26, -12], [-21, -15], [-15, -11], [-14, 2], [-16, 6], [-27, 6], [-24.5, -6]].map(([x, z]) => new THREE.Vector3(x, 0, z)), true), [["#e8558a", ""], [NA.yellow, "cap"], ["#2f6fb5", ""], [NA.white, "cap"], ["#3f8f5a", ""]], 0.006],
  ];
  ...
  for (const [x, z, n] of [[25, -10, 3], [19, -8, 2], [1, -4.5, 2], [-13, 11, 2], [-20, 4, 2]] as [number, number, number][]) for (let i = 0; i < n; i++) { const a = (i / n) * Math.PI * 2; place(american([NA.white, NA.denim, "#e8558a", NA.yellow][(i + Math.abs(x)) % 4], { cap: i === 0 ? "#2a2a2e" : undefined }), x + Math.cos(a) * 0.5, z + Math.sin(a) * 0.5, -a - Math.PI / 2); }
