...
function layoutSeasia({ group, tickers, place, tint, TOP }: LayoutCtx) {
  ...
  // ---------- Bangkok ----------
  for (const [x, z, rot] of [[-9, -20, 0.1], [-11, -23.5, -0.1], [2, -23.5, 0.1], [14, -1, 0.2]] as [number, number, number][]) place(tubeHouse("#f3e9d2", 2), x, z, rot);
  ...
  group.add(path([[0, -19.5], [6, -19.5], [11, -16], [11, -8], [10.5, 0], [4, -0.5], [1.5, -8], [0, -19.5]], 2.0, "#d3c8ad"));
  const tuks = [tukTuk("#2f6fb5"), tukTuk("#e8558a"), tukTuk("#3f8f5a")];
  tuks.forEach((tk) => group.add(tk));
  const bkk = new THREE.CatmullRomCurve3([[0, -19.5], [6, -19.5], [11, -16], [11, -8], [10.5, 0], [4, -0.5], [1.5, -8]].map(([x, z]) => new THREE.Vector3(x, 0, z)), true);
  ...
  // ---------- Hanoi ----------
  for (const [x, z, rot, c] of [[17.5, -10.5, 0.1, "#e9c46a"], [18.5, -7, -0.1, "#d9a86c"], [7, -13, 0.2, "#e9c46a"]] as [number, number, number, string][]) place(tubeHouse(c, 3), x, z, rot);
  ...
  tickers.push((t) => bikes.forEach((b, i) => { const lane = i % 2; const a = t * (0.32 + lane * 0.05) + Math.floor(i / 2) * (Math.PI * 2 / 3) + lane * 0.7; const r = 4.9 + lane * 1.2; b.position.set(14.5 + Math.cos(a) * r, 0, -22 + Math.sin(a) * r); b.rotation.y = -a; }));
  add(group, new THREE.Mesh(new THREE.RingGeometry(4.5, 6.5, 32), mat("#d3c8ad")), 14.5, TOP + 0.02, -22).rotation.x = -Math.PI / 2;
  ...
  const lane = new THREE.CatmullRomCurve3([[24, 10], [29, 6], [31, -3], [26.5, -12], [31, -19], [36, -8], [36, 9], [31, 14], [24, 14]].map(([x, z]) => new THREE.Vector3(x, 0, z)), true);
  ...
  const loops: [THREE.CatmullRomCurve3, [string, string][], number][] = [
    [new THREE.CatmullRomCurve3([[0, -19.5], [6, -19.5], [11, -16], [11, -8], [10.5, 0], [4, -0.5], [1.5, -8]].map(([x, z]) => new THREE.Vector3(x, 0, z)), true), [["#f4f1ea", ""], ["#e8558a", "nonLa"], ["#3f6fb5", ""], ["#2f5d3f", "nonLa"], ["#f2c14e", ""], ["#c0392b", ""]], 0.007],
    [new THREE.CatmullRomCurve3([[-14, -6.5], [-11.5, -2.5], [-12.5, 5], [-17, 10], [-24, 8], [-30, 5], [-33, -2], [-26, -6], [-20, -8]].map(([x, z]) => new THREE.Vector3(x, 0, z)), true), [["#2a2a2e", "nonLa"], ["#f4f1ea", "nonLa"], ["#3f6fb5", ""], ["#e8558a", "nonLa"], ["#7a4a3a", ""]], 0.006],
    [new THREE.CatmullRomCurve3([[13.5, -15.0], [16, -15], [21, -14.5], [21.5, -11], [21.5, -5.5], [16, -4.5], [14.5, -7], [14, -12]].map(([x, z]) => new THREE.Vector3(x, 0, z)), true), [["#f4f1ea", "nonLa"], ["#e8558a", ""], ["#3f6fb5", "nonLa"], ["#2a2a2e", ""]], 0.008],
  ];
  ...
  group.add(path([[-14, -6.5], [-11.5, -2.5], [-12.5, 5], [-17, 10], [-24, 8], [-30, 5], [-33, -2], [-26, -6], [-20, -8], [-14, -6.5]], 1.6, "#d9c7a0"));
  group.add(path([[13.5, -15.0], [16, -15], [21, -14.5], [21.5, -11], [21.5, -5.5], [16, -4.5], [14.5, -7], [14, -12], [13.5, -15.0]], 1.6, "#d3c8ad"));
